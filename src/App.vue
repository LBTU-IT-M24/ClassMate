...
const widgetVisibility = ref([true, false, false, false, false, false]);
const chooseUserType = (type) => {
    userType.value = type;
    userTypeChosen.value = true;
};
const toggleWidget = (index) => {
    widgetVisibility.value[index] = !widgetVisibility.value[index];
};
</script>
...
<template>
    <header v-if="userTypeChosen" class="bg-light">
        ...
        <div v-if="showOptions" class="options-dialog">
            <div class="dialog-content">
                <h5>Workspace Settings</h5>
                <div class="mb-3">
                    <label>Tools</label>
                    <ul>
                        <li><input type="checkbox" id="analogClock" v-model="widgetVisibility[0]" /> Analog Clock</li>
                        <li><input type="checkbox" id="timer" v-model="widgetVisibility[1]" /> Timer & Stopwatch</li>
                        <li><input type="checkbox" id="lessons" v-model="widgetVisibility[2]" /> Lessons</li>
                        <li><input type="checkbox" id="notes" v-model="widgetVisibility[3]" /> Notes</li>
                        <li><input type="checkbox" id="playlist" v-model="widgetVisibility[4]" /> Playlist</li>
                        <li><input type="checkbox" id="bookmarks" v-model="widgetVisibility[5]" /> Bookmarks</li>
                        <li><input type="checkbox" id="titleBar" v-model="widgetVisibility[6]" /> Title Bar</li>
                        <li>
                            <input type="checkbox" id="brainstormBoard" v-model="widgetVisibility[7]" /> Brainstorm
                            Board
                        </li>
                    </ul>
                </div>
                <button class="btn btn-primary" @click="showOptions = false">Save</button>
            </div>
        </div>
    </header>
